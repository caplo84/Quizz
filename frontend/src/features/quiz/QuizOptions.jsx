...
function QuizOptions({ option, optionIndex, userAnswer, setIsAnswered, question }) {
  ...
  // Get choice-specific content using option index
  const choiceCode = question?.choice_codes?.[optionIndex];
  const choiceCodeLanguage = question?.choice_code_languages?.[optionIndex];
  const choiceImageUrl = question?.choice_image_urls?.[optionIndex];
  const choiceImageAlt = question?.choice_image_alts?.[optionIndex];
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Choice Code Block */}
      {choiceCode && (
        <div className="ml-16 tablet:ml-12 mobile:ml-8">
          <CodeBlock 
            code={choiceCode} 
            language={choiceCodeLanguage}
            className="shadow-sm"
          />
        </div>
      )}
